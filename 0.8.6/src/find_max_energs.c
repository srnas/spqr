...
int main(int argc, char **argv) {
  ...
  double maxenerg=-1000, energ, nb_energ;
  int mc_block;
  ...
  for(i=mc_ini;i<mc_ini+mc_block;++i) {
    ...
    //energ=SA_eval_total_energy(nt_n, rx, ry, rz);
    //}
    //}
    energ=MC_get_energy(nt_n, rx, ry, rz, 3);//SA_eval_total_energy(nt_n, rx, ry, rz);
    nb_energ=MC_get_energy(nt_n, rx, ry, rz, 2);
    if(energ>maxenerg && nb_energ==0){
      maxenerg=energ;
      MC_min_energ_xyz_configuration(mc_n, rx, ry, rz, maxenerg, init);
