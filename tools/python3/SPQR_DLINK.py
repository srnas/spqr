...
print("SPQR_DLINK : detect links between secondary structure elements on a SPQR RNA structure.")
...
def check_pierce(tri, vlist):
    ...
    for segind in range(0,len(vlist)-1):
        intersc=False
        for coo in tri:
...
for lo in loops:
    ...
    if(fl==1):
        ...
        if(lend<NNT-1):
            lend=lend+1
        HAIRPINS.append(list(range(linit,lend+1)))
...
for nt in range(0,NNT-1):
    ist,ihp,iul,nist=-1,-1,-1,-1
    for st in range(0,len(STEMS)):
        if(nt in STEMS[st]):
            ist=st
        if(nt+1 in STEMS[st]):
            nist=st
    for hp in range(0,len(HAIRPINS)):
...
#USE THE FULL UNSTRUCTURED LOOPS FOR GAUSS INTEGRALS
FULLUNSTLS=[]
for iul in range(0,len(UNSTLS)):
    uloop=UNSTLS[iul]
    tloop=uloop
    lstart=uloop[0]
...
for st in STEMS:
    ...
    for nt in range(1,int(len(st)/2)):
    ...
    for nt in range(int(len(st)/2+1),len(st)):
        stc.append(np.array(COORDS[st[nt]][2]))
        stc.append(np.array(COORDS[st[nt]][1]))
...
ALLINDEXES=[]
if calctype=="g":
    print("Calculating with Gauss integrals")
    LOOPCOORDS=[]
    for hp in range(0,len(HPCOORDS)):
        LOOPCOORDS.append([HPCOORDS[hp],["hairpin ",hp]])
    for st in range(0,len(STCOORDS)):
        LOOPCOORDS.append([STCOORDS[st],["stem ", st]])
    for ul in range(0,len(FULLULCOORDS)):
    ...
    for iloop1 in range(0,len(LOOPCOORDS)):
        for iloop2 in range(iloop1+1,len(LOOPCOORDS)):
    ...
    #    for iloop2 in xrange(iloop1+1,0):
            ...
            if(dist and sameflag):
                ...
                for pa1 in range(0,len(LOOPCOORDS[iloop1][0])-1):
                    for pa2 in range(0,len(LOOPCOORDS[iloop2][0])-1):
                ...
                if(fabs(GAUSSINT)>0.5):
                    ...
                    floop1=indexes[0]
                    floop2=indexes[1]
                    TEMPSTR="LINK DETECTED BETWEEN LOOPS: ("+str(floop1[1])+") : "+str(floop1[0])+" and ("+str(floop2[1]) + ") : " +str(floop2[0])+" . Gauss integral = "+str(GAUSSINT)
                    print(TEMPSTR)
...
if calctype=="p":
    print("Calculating with piercing method")
    ...
    for pair in REDUNDANTINDEXES:
        if(pair not in PIERCEINDEXES and [pair[1],pair[0]] not in PIERCEINDEXES):
            ...
            TEMPSTR="LINK DETECTED BETWEEN LOOPS: ("+str(pair[0][1])+") : "+str(pair[0][0])+ "  and  ("+str(pair[1][1])+") : "+str(pair[1][0])
            print(TEMPSTR)
            LOGDATA.append(TEMPSTR)
    ...
    if len(LOOPLIST)>0 :
        ...
        for loo in LOOPLIST:
            print(str(loo[0])+" "+str(loo[1]))
        for li in LINKOUTPUT:
            print(str(LOOPLIST.index(li[0]))+" "+str(LOOPLIST.index(li[1])))
        OUTFILE.close()
