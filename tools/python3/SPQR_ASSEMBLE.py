...
rawseq=[]
if(args.sequences!=""):
    for i in range(0,len(args.sequences)):
        if(args.sequences[i]!="&"):
...
refcoord=[]
for nt1 in range(0,NBA):
    wr2=[]
    for nt2 in range(0,NBA):
        ALLFILE=[]
...
for strand in range(0,len(seq)):
    ...
    if(PRDUPLEX):
        for nt in range(0,len(seq[strand])):
...
if(ssflag):
    ...
    orig_stdout=sys.stdout
    sys.stdout=frags 
    for ST in range(0,len(STACKSEQS)):
