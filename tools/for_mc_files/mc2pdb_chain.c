...
void MC_write_pdb(FILE * , int, double *, double *, double *, double , double , int *, int *, int *, int *);
...
int main(int argc, char **argv){
  ...
  for(i=0;i<nconfs;i++){
    ...
    if(i>=confini){
      ...
      MC_write_pdb(tempconf,  MC_N,  posx, posy, posz,  energy_t, temperature, glyc, puck, chains, type);
...
void MC_write_pdb(FILE * pdbfile, int mc_n, double *rx, double *ry, double *rz, double energy_t, double temperature, int *mc_glyc, int *mc_puck, int *mc_chains, int *mc_types){
