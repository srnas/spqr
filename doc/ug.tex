...
\begin{itemize}
\item \begin{verbatim} SPQR_MC [-i job_index]\end{verbatim} 
  Runs a simple Monte Carlo simulation at constant temperature. Requires the initial condition stored in the \verb pdb_inits ~directory, which can be created with the scripts described in the Tools section.
...
\item \begin{verbatim} SPQR_SA [-i job index] \end{verbatim}
...
Then, move \verb ermsd_frags.lst ~and \verb SPQR_MC ~to the \verb example ~directory. Finally, run the simulation.
