...
void SA_read_params(double *tmax, double *tmin, double *tfac, int *sa_step,  int *NT, double *prev_energ, double *sfac, int *resc_times, int mpi_id){
  ...
  char filename[256];
  sprintf(filename, "input.spqr");
  FILE *SA_PARAMS;
  //we assume that input.spqr already exists
  if((SA_PARAMS=fopen(filename, "r"))==NULL){
    printf("SIMULATED ANNEALING: input.spqr file not found.\n");
    exit(ERR_INPUT);
  }
	...
	if(line[0]=='S' && line[1]=='A' && line[2]=='_'){
	  ...
	  sscanf(line, "%s", s);
	  if(!strcmp(s, "SA_TINI")) {
	    if(sscanf(line, "%s %lf", s2, &temp1)!=2){printf("Invalid value of SA_TINI (initial temperature) in input.spqr\n");exit(ERR_INPUT);}
	    cnt++;
	  }
	  else if(!strcmp(s, "SA_TMIN")) {
	    if(sscanf(line, "%s %lf", s2, &temp2)!=2){printf("Invalid value of SA_TMIN (minimum temperature) in input.spqr\n");exit(ERR_INPUT);}
	  }
	  else if(!strcmp(s, "SA_TFAC")) {
	    if(sscanf(line, "%s %lf", s2, &temp3)!=2){printf("Invalid value of SA_TFAC (temperature factor) in input.spqr\n");exit(ERR_INPUT);}
	  }
	  else if(!strcmp(s, "SA_STEP")) {
	    if(sscanf(line, "%s %d", s2, &temp8)!=2){printf("Invalid value of SA_STEP (current number of annealing step) in input.spqr\n");exit(ERR_INPUT);}
	  }
	  else if(!strcmp(s, "SA_NT")) {
	    if(sscanf(line, "%s %d", s2, &temp4)!=2){printf("Invalid value of SA_NT (total number of annealing iterations)  in input.spqr\n");exit(ERR_INPUT);}
	  }
	  else if(!strcmp(s, "SA_PREENERG")) {
	    if(sscanf(line, "%s %lf", s2, &temp5)!=2){printf("Invalid value of SA_PREENERG (energy of previous annealing step) in input.spqr\n");exit(ERR_INPUT);}
	  }
	  else if(!strcmp(s, "SA_SFAC")) {
	    if(sscanf(line, "%s %lf", s2, &temp6)!=2){printf("Invalid value of SA_SFAC (Monte Carlo step factor) in input.spqr\n");exit(ERR_INPUT);}
	  }
	  else if(!strcmp(s, "SA_RTIMES")) {
	    if(sscanf(line, "%s %d", s2, &temp7)!=2){printf("Invalid value of SA_RTIMES (current number of rescaling times) in input.spqr\n");exit(ERR_INPUT);}
	  }
	}
    }
    *tmax=temp1;
