...
int main(int argc, char **argv) {
  ...
  //we read from input.spqr or update the data found in the checkpoint file
  ...
  printf("Processor %d starting from time %d\n", mpi_id, mc_i0);
  sa_temp=sa_tmax;
  ...
  printf("PROC %d , INITIAL ENERGY IS %lf\n", mpi_id, energy_t);
#ifdef ERMSD
  ...
  for(ann_step=sa_ini;ann_step<sa_NT;ann_step++){
	...
	MC_save_configuration(mc_n, rx, ry, rz, energy_t);
      ...
      if(((i+1)%(mc_chkp_steps))==0 && i<mc_iter){
	SA_params_to_arr(sa_temp, sa_tmin, sa_tfac, ann_step, sa_NT, sa_prev_energ, sa_sfac, sa_resc_times, SA_DATA);
	MC_save_checkpoint(mc_n, rx, ry, rz, (i+1)+ann_step*mc_iter, energy_t, mpi_id, SA_DATA); //this writes a binary checkpoint
      }
    }
    ...
    MC_save_checkpoint(mc_n, rx, ry, rz, mc_iter+ann_step*mc_iter, energy_t, mpi_id, SA_DATA); //this writes a binary checkpoint
