...
void init_loop_vsites(int nloops, double *posx, double *posy, double *posz){
  int i,j,nsites=1,d,iloop,ntind,nt,centroid,nt1,nt2;
  mc_loop_vsites=(double ***)malloc(nloops*sizeof(double **));
  for(i=0;i<nloops;i++)
    mc_loop_vsites[i]=(double **)malloc(sizeof(double *));
  
  for(i=0;i<nloops;i++)
    for(j=0;j<nsites;j++){
      mc_loop_vsites[i][j]=(double *)malloc(DIM*sizeof(double));
      for(d=0;d<DIM;d++)
	mc_loop_vsites[i][j][d]=0.0;
      
    }
  
  //calculate centroids as the virtual sites, which won't be updated along the simulation
  for(iloop=0;iloop<nloops;iloop++){
    /* for(ntind=0;ntind<mc_loop_size[iloop];ntind++){ */
    /*   nt=mc_loops[iloop][ntind]; */
    /*   mc_loop_vsites[iloop][0][0]+=(posx[nt*N_PARTS_PER_NT+IPHO]+posx[nt*N_PARTS_PER_NT+ISUG]); */
    /*   mc_loop_vsites[iloop][0][1]+=(posy[nt*N_PARTS_PER_NT+IPHO]+posy[nt*N_PARTS_PER_NT+ISUG]); */
    /*   mc_loop_vsites[iloop][0][2]+=(posz[nt*N_PARTS_PER_NT+IPHO]+posz[nt*N_PARTS_PER_NT+ISUG]); */
    /* } */
    
    /* for(d=0;d<DIM;d++) */
    /*   mc_loop_vsites[iloop][0][d]/=(2.0*(double)mc_loop_size[iloop]); */
    if(mc_loop_size[iloop]%2==0){
      nt1=(int)((mc_loops[iloop][0]+mc_loops[iloop][mc_loop_size[iloop]-1])/2.0);
      nt2=nt1+1;
      mc_loop_vsites[iloop][0][0]=(posx[nt1*N_PARTS_PER_NT+IPHO]+posx[nt1*N_PARTS_PER_NT+ISUG]+posx[nt2*N_PARTS_PER_NT+IPHO]+posx[nt2*N_PARTS_PER_NT+ISUG])/4.0;
      mc_loop_vsites[iloop][0][1]=(posy[nt1*N_PARTS_PER_NT+IPHO]+posy[nt1*N_PARTS_PER_NT+ISUG]+posy[nt2*N_PARTS_PER_NT+IPHO]+posy[nt2*N_PARTS_PER_NT+ISUG])/4.0;
      mc_loop_vsites[iloop][0][2]=(posz[nt1*N_PARTS_PER_NT+IPHO]+posz[nt1*N_PARTS_PER_NT+ISUG]+posz[nt2*N_PARTS_PER_NT+IPHO]+posz[nt2*N_PARTS_PER_NT+ISUG])/4.0;
    } else {
      nt1=(int)((mc_loops[iloop][0]+mc_loops[iloop][mc_loop_size[iloop]-1])/2.0);
      mc_loop_vsites[iloop][0][0]=(posx[nt1*N_PARTS_PER_NT+IPHO]+posx[nt1*N_PARTS_PER_NT+ISUG])/2.0;
      mc_loop_vsites[iloop][0][1]=(posy[nt1*N_PARTS_PER_NT+IPHO]+posy[nt1*N_PARTS_PER_NT+ISUG])/2.0;
      mc_loop_vsites[iloop][0][2]=(posz[nt1*N_PARTS_PER_NT+IPHO]+posz[nt1*N_PARTS_PER_NT+ISUG])/2.0;
    }
  }
  printf("Loop virtual sites initialized.\n");
}

void MC_set_linked_loops(int nt_n, double *posx, double *posy, double *posz){
  //we read a file like the ermsd frags
  int i,j,k,d,plindex;
  int tmpout, temp,initc=0,dinit=0;
  char *ptmpout;
  FILE *lnkdloopsfile;
  char filename[256];
  ...
  if((lnkdloopsfile=fopen(filename, "r"))==NULL){
    ...
    mc_loop_strand_edges=(int***)malloc(sizeof(int**)*mc_N_loops);
    mc_N_clpairs=(int*)malloc(sizeof(int)*mc_N_loops);
    mc_N_strands=(int*)malloc(sizeof(int)*mc_N_loops);
    
    for(i=0;i<mc_N_links;i++) mc_links[i]=(int *)malloc(sizeof(int)*2);
    for(i=0;i<mc_N_loops;i++){
      ptmpout=fgets(loopline,MAXSTR,lnkdloopsfile);
      sscanf(loopline,"%s",ltyp);
      if(!strcmp(ltyp,"hp")) { mc_loop_type[i]=LOOP_HP; mc_N_clpairs[i]=1;mc_N_strands[i]=1;}
      else if(!strcmp(ltyp,"st")) { mc_loop_type[i]=LOOP_ST; mc_N_clpairs[i]=2;mc_N_strands[i]=2;}
      else if(!strcmp(ltyp,"il")) { mc_loop_type[i]=LOOP_IL; mc_N_clpairs[i]=2;mc_N_strands[i]=1;}//they do have two nucleotides
      else if(!strcmp(ltyp,"ju")) { mc_loop_type[i]=LOOP_JU; sscanf(loopline, "%s %d", ltyp,&temp); mc_N_clpairs[i]=temp;mc_N_strands[i]=temp;if(mc_N_strands[i]==1) mc_N_clpairs[i]=2;}
      else {printf("Wrong loop type at file %s!\n%s", filename,ltyp);exit(ERR_INPUT);}
      
      mc_loop_strand_edges[i]=(int**)malloc(mc_N_strands[i]*sizeof(int*));
      
      if(mc_loop_type[i]==LOOP_HP || mc_loop_type[i]==LOOP_IL) {sscanf(loopline,"%s %d %d", ltyp,  &ini1,&end1);mc_loop_size[i]=end1-ini1+1;
	mc_loop_strand_edges[i][0]=(int*)malloc(2*sizeof(int));
	mc_loop_strand_edges[i][0][0]=ini1;mc_loop_strand_edges[i][0][1]=end1;}
      else if(mc_loop_type[i]==LOOP_ST){sscanf(loopline,"%s %d %d %d %d", ltyp, &ini1,&end1,&ini2,&end2); mc_loop_size[i]=end1-ini1+1+end2-ini2+1;
	mc_loop_strand_edges[i][0]=(int*)malloc(2*sizeof(int));mc_loop_strand_edges[i][1]=(int*)malloc(2*sizeof(int));
	mc_loop_strand_edges[i][0][0]=ini1;mc_loop_strand_edges[i][0][1]=end1;mc_loop_strand_edges[i][1][0]=ini2;mc_loop_strand_edges[i][1][1]=end2;
      }
      else if(mc_loop_type[i]==LOOP_JU) {sscanf(loopline,"%s %d %d %d%n", ltyp, &temp,&ini1,&end1,&dinit); mc_loop_size[i]=end1-ini1+1;
	initc=0;
	mc_loop_strand_edges[i][0]=(int*)malloc(2*sizeof(int));
	mc_loop_strand_edges[i][0][0]=ini1;mc_loop_strand_edges[i][0][1]=end1;
	for(k=1;k<mc_N_strands[i];k++){
	  initc+=dinit;
	  sscanf(loopline+initc, "%d %d%n", &ini2, &end2,&dinit);
	  mc_loop_size[i]+=end2-ini2+1;
	  mc_loop_strand_edges[i][k]=(int*)malloc(2*sizeof(int));
	  mc_loop_strand_edges[i][k][0]=ini2;mc_loop_strand_edges[i][k][1]=end2;
	}
      }
      mc_loops[i]=(int*)malloc(sizeof(int)*mc_loop_size[i]);
      if(mc_loop_type[i]==LOOP_HP || mc_loop_type[i]==LOOP_IL || mc_loop_type[i]==LOOP_ST){
	...
	if(mc_loop_type[i]==LOOP_ST){
      ...
      if(mc_loop_type[i]==LOOP_JU){
	plindex=0;
	for(j=mc_loop_strand_edges[i][0][0];j<mc_loop_strand_edges[i][0][1]+1 ;j++){
	  //mc_loops[i][j-mc_loop_strand_edges[i][k][0] + (mc_loop_strand_edges[i][k-1][1]+1 - mc_loop_strand_edges[i][k-1][0])] = j;
	  //plindex=j-mc_loop_strand_edges[i][0][0];
	  mc_loops[i][plindex] = j;
	  in_link[j]++;
	  plindex++;
	}
	for(k=1;k<mc_N_strands[i];k++){
	  for(j=mc_loop_strand_edges[i][k][0];j<mc_loop_strand_edges[i][k][1]+1;j++){
	    //plindex=j-mc_loop_strand_edges[i][k][0] + (mc_loop_strand_edges[i][k-1][1]+1 - mc_loop_strand_edges[i][k-1][0]);
	    mc_loops[i][plindex]=j;
	    in_link[j]++;
	    plindex++;
	  }
      ...
      mc_loop_clpair[i]=(double *)malloc(mc_N_clpairs[i]*sizeof(double)*DIM);
    }
  ...
  init_loop_vsites(mc_N_loops,posx,posy,posz);
...
void mc_update_loop(int iloop, int nt_c, double *rx, double *ry, double *rz){
  int d, ntind, nt1,nt2,nt3,nt4,i,j;
  double pos1[DIM],pos2[DIM],pos3[DIM],pos4[DIM];
  for(d=0;d<DIM;d++){
    mc_loop_CM[iloop][d]=0;
    for(i=0;i<mc_N_clpairs[iloop];i++){
      mc_loop_clpair[iloop][i*DIM+d]=0;
    }
  }
  ...
  if(mc_loop_type[iloop]==LOOP_HP){
    ...
    for(d=0;d<DIM;d++){
      ...
      mc_loop_clpair[iloop][DIM+d]=pos2[d];
    }
    ...
    for(d=0;d<DIM;d++){
      ...
      mc_loop_clpair[iloop][DIM+d]=0.5*(pos2[d]+pos3[d]);
    }
  } else if (mc_loop_type[iloop]==LOOP_JU){
    if(mc_N_strands[iloop]==1){
      nt1=mc_loop_strand_edges[iloop][ 0 ][0]; //first nt of junction
      nt2=mc_loop_strand_edges[iloop][ mc_N_strands[iloop] - 1][1]; //last nt of junction
      get_real_sugpos(nt1,nt_c,rx,ry,rz,pos1);
      get_real_sugpos(nt2,nt_c,rx,ry,rz,pos2);
      for(d=0;d<DIM;d++){
      mc_loop_clpair[iloop][d]=pos1[d];
      ...
      nt1=mc_loop_strand_edges[iloop][ mc_N_strands[iloop] - 1][1]; //last nt of junction
      nt2=mc_loop_strand_edges[iloop][ 0 ][0]; //first nt of junction
      get_real_sugpos(nt1,nt_c,rx,ry,rz,pos1);
      get_real_sugpos(nt2,nt_c,rx,ry,rz,pos2);
      for(d=0;d<DIM;d++)
	mc_loop_clpair[iloop][d]=0.5*(pos1[d]+pos2[d]);
      for(i=1;i<mc_N_strands[iloop];i++){
	nt1=mc_loop_strand_edges[iloop][ i-1 ][1]; //last nt of previous strand
	nt2=mc_loop_strand_edges[iloop][ i   ][0]; //first nt of this strand
	get_real_sugpos(nt1,nt_c,rx,ry,rz,pos1);
	get_real_sugpos(nt2,nt_c,rx,ry,rz,pos2);
	
	for(d=0;d<DIM;d++)
	  mc_loop_clpair[iloop][DIM*i+d]=0.5*(pos1[d]+pos2[d]);
      }
    }
  }
  else {
...
double calc_link_energy(int nt_c, double *rx, double *ry, double *rz){
  ...
  for(ilink=0;ilink<mc_N_links;ilink++){
    if(nt_is_in_link(ilink,nt_c)==1) {
      ...
      d1sq=0;d2sq=0;
      if(mc_loop_type[l1]==LOOP_IL && mc_loop_type[l2]==LOOP_IL){
	//here is a bit different: we calculate the distance between the test particle and the virtual sites
	d1sq+=SQ(mc_loop_vsites[l1][0][0]-get_unf_coo_temp_x(nt_c*N_PARTS_PER_NT+IPHO));
	d1sq+=SQ(mc_loop_vsites[l1][0][1]-get_unf_coo_temp_y(nt_c*N_PARTS_PER_NT+IPHO));
	d1sq+=SQ(mc_loop_vsites[l1][0][2]-get_unf_coo_temp_z(nt_c*N_PARTS_PER_NT+IPHO));

	d2sq+=SQ(mc_loop_vsites[l2][0][0]-get_unf_coo_temp_x(nt_c*N_PARTS_PER_NT+IPHO));
	d2sq+=SQ(mc_loop_vsites[l2][0][1]-get_unf_coo_temp_y(nt_c*N_PARTS_PER_NT+IPHO));
	d2sq+=SQ(mc_loop_vsites[l2][0][2]-get_unf_coo_temp_z(nt_c*N_PARTS_PER_NT+IPHO));
	
	//if test particle belongs to l1 : l1 is repulsive and l2 attractive
	if(nt_is_in_loop(l1,nt_c)){
	  energ+=-LOOP_K_cmcm*sqrt(d1sq);
	  energ+= LOOP_K_cmcm*sqrt(d2sq);
	}
	//if test particle belongs to l2 : l2 is repulsive and l1attractive
	else if(nt_is_in_loop(l2,nt_c)){
	  energ+= LOOP_K_cmcm*sqrt(d1sq);
	  energ+=-LOOP_K_cmcm*sqrt(d2sq);
	}
      }
      else{
	//loop over closing points and centers of mmass
	for(i=0;i<mc_N_clpairs[l2];i++){
	  d1sq=0;
	  for(d=0;d<DIM;d++)
	    d1sq+=SQ(mc_loop_CM[l1][d]-mc_loop_clpair[l2][i*DIM+d]);
	  energ+=-LOOP_K_cmclp*sqrt(d1sq);
	}
	for(i=0;i<mc_N_clpairs[l1];i++){
	  d2sq=0;
	  for(d=0;d<DIM;d++)
	    d2sq+=SQ(mc_loop_CM[l2][d]-mc_loop_clpair[l1][i*DIM+d]);
	...
	if(mc_loop_type[l1]==LOOP_ST || mc_loop_type[l2]==LOOP_ST || mc_loop_type[l1]==LOOP_JU || mc_loop_type[l2]==LOOP_JU  ){
	  d1sq=0;
	  for(d=0;d<DIM;d++)
...
double MC_wall_energy(double px, double py, double pz){
  ...
  for(w=0;w<N_WALLS;w++){
	...
	ret+=repd3*repd3*repd3-wall_epsilon[w]*exp(-dist/wall_sigma[w]);//debye huckel
      ...
      else if(WALL_TYPE[w]==1) //mie
	ret+=wall_epsilon[w]*d3*(d3*d3-1);
